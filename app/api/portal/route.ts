...
export async function POST(request: Request) {
  try {
    ...
    if (action === "create_project") {
      ...
      const data = { kind: textValue(body.kind, "Document"), label: textValue(body.label, "Production update"), date: new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" }).format(new Date()), status: "Shared" };
