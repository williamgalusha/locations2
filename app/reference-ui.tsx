...
function ClientBudgetDocument({ version, back }: { version: PortalData["budgetVersions"][number]; back: () => void }) {
  const sections = [...new Set(version.snapshot.map((line) => line.section_code || "A"))];
  return <section className="client-budget-view">
    ...
    <article className="client-budget-document">
      <header><div><span>PUBLISHED ESTIMATE</span><h2>{version.name}</h2></div><div><span>VERSION TOTAL</span><strong>{money.format(snapshotTotal(version.snapshot))}</strong><small>{new Date(version.created_at).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</small></div></header>
      <div className="client-budget-document-head"><span>LINE</span><span>ITEM / DESCRIPTION</span><span>AMOUNT</span></div>
      {sections.map((section) => { const lines = version.snapshot.filter((line) => (line.section_code || "A") === section); return <section key={section}><header><b>{section}</b><strong>{lines[0]?.category || "PRODUCTION"}</strong><span>{money.format(snapshotTotal(lines))}</span></header>{lines.map((line, index) => <div className="client-budget-document-line" key={line.id}><b>{line.item_code || `${section}${index + 1}`}</b><span><strong>{line.item_name || line.category}</strong><small>{line.description}</small></span><i>{line.is_na ? "N/A" : money.format(cleanNumber(line.estimate))}</i></div>)}</section>; })}
      <footer><span>CONFIRMED PRODUCTION ESTIMATE</span><strong>TOTAL</strong><b>{money.format(snapshotTotal(version.snapshot))}</b></footer>
    </article>
  </section>;
}
...
function ClientBudgets({ versions, shares }: { versions: PortalData["budgetVersions"]; shares: PortalData["records"] }) {
  ...
  if (mode === "view" && selected) return <ClientBudgetDocument version={selected} back={() => setMode("library")} />;
  ...
  return <section className="client-budget-library">
    <header><div><span>PUBLISHED BUDGETS</span><h2>{published.length ? `${published.length} CLIENT-READY VERSION${published.length === 1 ? "" : "S"}` : "NO BUDGETS PUBLISHED"}</h2><p>Open or download any shared estimate, or compare two published versions.</p></div><button disabled={published.length < 2} onClick={() => setMode("compare")}>COMPARE TWO VERSIONS →</button></header>
...
export function ReferenceClientPortal({ data, totals, preview, setPreview, publish, clientOnly = false }: { data: PortalData; totals: BudgetTotals; preview: boolean; setPreview: (value: boolean) => void; publish: Mutate; clientOnly?: boolean }) {
  ...
  if (locationPdfView) return <LocationPresentation project={data.project} locations={data.locations} close={() => setLocationPdfView(false)} mutate={publish} />;
