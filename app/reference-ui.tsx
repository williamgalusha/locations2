...
import type { BudgetLine, BudgetSnapshot, BudgetVersion, Location, Mutate, PortalData, Project, User } from "./production-portal";
...
function EstimateSignoffComposer({ version, suggestedAmount, mutate, close }: { version: BudgetVersion; suggestedAmount: number; mutate: Mutate; close: () => void }) {
  const [signedOffAt, setSignedOffAt] = useState(version.signed_off_at || new Date().toISOString().slice(0, 10));
  const [signedOffBy, setSignedOffBy] = useState(version.signed_off_by || "");
  const [signedOffAmount, setSignedOffAmount] = useState(String(version.signed_off_amount || suggestedAmount));
  const [billingPercent, setBillingPercent] = useState(String(version.billing_percent || (version.status.startsWith("overage") ? 100 : 50)));
  const [paymentTerms, setPaymentTerms] = useState(version.payment_terms || (version.status.startsWith("overage") ? "100% upon approval · Net 30" : "50% advance · Balance upon completion · Net 30"));
  const [saving, setSaving] = useState(false);
  const updateTerms = (value: string) => {
    setPaymentTerms(value);
    const detected = value.match(/(\d+(?:\.\d+)?)\s*%/)?.[1];
    if (detected) setBillingPercent(detected);
  };
  const save = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true);
    const saved = await mutate({ action: "update_budget_signoff", id: version.id, signedOff: true, signedOffAt, signedOffBy, signedOffAmount: cleanNumber(signedOffAmount), billingPercent: cleanNumber(billingPercent), paymentTerms }, `${version.name} marked signed off`);
    setSaving(false); if (saved) close();
  };
  return <div className="modal-backdrop signoff-composer-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><form className="signoff-composer" onSubmit={save} role="dialog" aria-modal="true" aria-label={`Sign off ${version.name}`}><header><div><span>ADMIN · APPROVAL RECORD</span><h2>ESTIMATE SIGN-OFF</h2><p>{version.status.startsWith("overage") ? "OVERAGE" : "ESTIMATE"} · {version.name}</p></div><button type="button" onClick={close} aria-label="Close sign-off editor">×</button></header><section className="signoff-fields"><label>SIGNED-OFF DATE<input type="date" value={signedOffAt} onChange={(event) => setSignedOffAt(event.target.value)} required /></label><label>SIGNED OFF BY<input value={signedOffBy} onChange={(event) => setSignedOffBy(event.target.value)} placeholder="Client, agency or approver" required /></label><label>APPROVED AMOUNT<input type="number" min="0" step="0.01" value={signedOffAmount} onChange={(event) => setSignedOffAmount(event.target.value)} required /></label><label>ADVANCE / INVOICE %<input type="number" min="0" max="100" step="0.01" value={billingPercent} onChange={(event) => setBillingPercent(event.target.value)} required /></label><label className="wide">PAYMENT TERMS<textarea value={paymentTerms} onChange={(event) => updateTerms(event.target.value)} placeholder="50% advance · Balance upon completion · Net 30" /></label></section><section className="signoff-calculation"><span>INVOICE SUGGESTION</span><strong>{money.format(cleanNumber(signedOffAmount) * cleanNumber(billingPercent) / 100)}</strong><p>{money.format(cleanNumber(signedOffAmount))} signed off × {cleanNumber(billingPercent)}%</p></section><footer>{version.signed_off ? <button type="button" className="remove" onClick={async () => { if (!window.confirm(`Remove the sign-off from ${version.name}?`)) return; setSaving(true); const removed = await mutate({ action: "update_budget_signoff", id: version.id, signedOff: false }, `${version.name} sign-off removed`); setSaving(false); if (removed) close(); }}>REMOVE SIGN-OFF</button> : <span />}<button type="button" onClick={close}>CANCEL</button><button className="solid" disabled={saving}>{saving ? "SAVING…" : version.signed_off ? "UPDATE SIGN-OFF" : "MARK SIGNED OFF"}</button></footer></form></div>;
}
...
export function ReferenceBudgetView({ data, lines, totals, openComposer, mutate, auditBudget, auditing }: { data: PortalData; lines: BudgetLine[]; totals: BudgetTotals; expenses: unknown[]; openComposer: () => void; mutate: Mutate; auditBudget: () => void; auditing: boolean }) {
  ...
  const [signingVersion, setSigningVersion] = useState<BudgetVersion | null>(null);
  ...
  return <section className="reference-budget-page">
    ...
    <section className="budget-version-manager budget-noprint">
      ...
      <div className="budget-version-block"><div className="budget-version-current"><span>CONFIRMED BUDGET</span><strong>{confirmed?.name || "WORKING ESTIMATE"}</strong>{!readOnly && <i>EDITING</i>}<b>{money.format(grand)}</b>{data.viewer.isAdmin && confirmed && <button className={confirmed.signed_off ? "budget-signoff-button signed" : "budget-signoff-button"} onClick={() => setSigningVersion(confirmed)}>{confirmed.signed_off ? `SIGNED OFF · ${money.format(Number(confirmed.signed_off_amount))} ✓` : "MARK SIGNED OFF"}</button>}<button onClick={() => mutate({ action: "save_budget_version", confirm: true }, "Budget saved as a new confirmed version")}>＋ SAVE AS NEW VERSION</button></div>
      ...
      <div className="budget-overage-block"><div><strong>CONFIRMED OVERAGES</strong><button onClick={() => mutate({ action: "save_budget_version", kind: "overage", confirm: true }, "New overage saved")}>＋ SAVE AS NEW OVERAGE</button></div>{confirmedOverages.map((version) => <div className="budget-overage-row" key={version.id}><i>CONFIRMED ✓</i><strong>OVERAGE — {version.name}</strong><span>{version.signed_off ? `SIGNED OFF · ${money.format(Number(version.signed_off_amount))}` : money.format(snapshotTotal(version.snapshot))}</span><footer>{data.viewer.isAdmin && <button className={version.signed_off ? "budget-signoff-button signed" : "budget-signoff-button"} onClick={() => setSigningVersion(version)}>{version.signed_off ? "EDIT SIGN-OFF" : "MARK SIGNED OFF"}</button>}<button onClick={() => setViewingVersionId(version.id)}>VIEW</button><button onClick={() => mutate({ action: "set_budget_version_status", id: version.id, status: "overage_archived" }, "Overage archived")}>ARCHIVE</button><button onClick={() => mutate({ action: "delete_budget_version", id: version.id }, "Overage deleted")}>×</button></footer></div>)}{!confirmedOverages.length && <p>No confirmed overages yet. Build an overage scenario below, then save it here.</p>}<button className="budget-history-toggle" onClick={() => setOverageOpen((value) => !value)}>▾ <strong>OVERAGE HISTORY</strong> <span>{archivedOverages.length} archived · view only</span></button>{overageOpen && <div className="budget-version-list">{archivedOverages.map((version) => <div key={version.id}><strong>OVERAGE — {version.name}</strong><span>{money.format(snapshotTotal(version.snapshot))}</span><footer><button onClick={() => setViewingVersionId(version.id)}>VIEW</button><button className="solid" onClick={() => reactivate(version.id, true)}>REACTIVATE</button><button onClick={() => mutate({ action: "delete_budget_version", id: version.id }, "Overage deleted")}>×</button></footer></div>)}</div>}</div>
    </section>
    {signingVersion && <EstimateSignoffComposer version={signingVersion} suggestedAmount={signingVersion.status.startsWith("overage") ? snapshotTotal(signingVersion.snapshot) : snapshotTotal(signingVersion.snapshot) * (1 + markup / 100 + insurance / 100)} mutate={mutate} close={() => setSigningVersion(null)} />}
