...
function ClientBudgetDocument({ project, version, back }: { project: Project; version: PortalData["budgetVersions"][number]; back: () => void }) {
  ...
  return <section className="client-budget-view">
    <div className="client-budget-view-actions"><button onClick={back}>← ALL PUBLISHED BUDGETS</button><button onClick={printClientBudget}>DOWNLOAD BUDGET PDF ↓</button></div>
    <article className="budget-document" data-budget-document>
      ...
      {sections.map((section) => <section className="estimate-section" id={`client-budget-${section.code}`} key={section.code}><header><b>{section.code}</b><input value={section.name} readOnly aria-label={`Section ${section.code} name`} /><span>RATE × QTY × DAYS + VAT / PAYROLL %</span></header>{!section.na && <><div className="estimate-columns"><span /><span>ITEM</span><span>DESCRIPTION</span><span>RATE</span><span>QTY</span><span>DAYS</span><span>VAT / PAYROLL</span><span>TOTAL</span></div>{section.rows.map((line, index) => <BudgetEditableRow key={line.id} line={line} displayCode={`${section.code}${index + 1}`} readOnly mutate={noMutation} dragStart={noDrag} drop={noDrag} />)}</>}{section.na && <div className="estimate-na"><span /><input value={section.note || "N/A — Not Applicable to This Estimate"} readOnly /><b>—</b></div>}<footer><span /><strong>SECTION {section.code} TOTAL</strong><span>{money.format(section.na ? 0 : section.rows.reduce((sum, line) => sum + line.total, 0))}</span></footer></section>)}
      <div className="estimate-thanks">Thank you!</div><footer className="estimate-footer"><span>BILL, INC.</span><span>PRODUCTION ESTIMATE</span><span>65 MOTT ST, #4, NEW YORK, NY 10013</span></footer>
    </article>
  </section>;
}
...
function ClientBudgets({ project, versions, shares }: { project: Project; versions: PortalData["budgetVersions"]; shares: PortalData["records"] }) {
  ...
  if (mode === "view" && selected) return <ClientBudgetDocument project={project} version={selected} back={() => setMode("library")} />;
...
function ClientBudgetWorkspace({ project, versions, shares }: { project: Project; versions: PortalData["budgetVersions"]; shares: PortalData["records"] }) {
...
export function ReferenceLocationsView({ project, projects, locations, switchProject, openGlobalLibrary, mutate }: { project: Project; projects: Project[]; locations: Location[]; switchProject: (projectId: string) => Promise<void>; openGlobalLibrary: () => void; open: () => void; mutate: Mutate }) {
